...
if not os.path.exists(dir_name):
    ...
    # Save plot
    plt.savefig(f'train_vs_test_loss_{plot_id}.png', dpi=300, bbox_inches='tight')
    print(f"\nLoss plot saved as 'train_vs_test_loss_{plot_id}.png'")

    # ====== ACCURACY PLOT ======
    plt.figure(figsize=(10, 6))
    
    plt.plot(hidden_dims, test_accs, linestyle='-', 
                linewidth=2, markersize=8, label=f'{label} - Test Accuracy', alpha=0.8)
    plt.plot(hidden_dims, train_accs, linestyle='--', 
                linewidth=2, markersize=8, label=f'{label} - Train Accuracy', alpha=0.8)

    # Formatting
    plt.xlabel('Hidden Dimension', fontsize=13)
    plt.ylabel('Accuracy (%)', fontsize=13)
    plt.title('Train vs Test Accuracy Across Hidden Dimensions', fontsize=15, fontweight='bold')
    plt.legend(fontsize=10, loc='best')
    plt.grid(True, alpha=0.3)
    plt.xscale('log')
    plt.yscale('linear')
    ...
    # Save plot
    plt.savefig(f'train_vs_test_accuracy_{plot_id}.png', dpi=300, bbox_inches='tight')
    print(f"Accuracy plot saved as 'train_vs_test_accuracy_{plot_id}.png'")
