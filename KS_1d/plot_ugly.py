...
model_dir = 'overfit'
epoch = 50
directory = f'/glade/derecho/scratch/tsatoperry/GAD/KS_1d/1_layer/{model_dir}/singular_values/'

filename = f'ugly_{model_dir}.png'

# Regex to match the new filename pattern
...
ax.set_title(f"Singular Values KS 1D AR MLP ({model_dir})")
ax.set_xlabel("Index")
ax.set_ylabel("Value")
...
# Add colorbar with width labels (no multiplication by 64 needed here)
sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
cbar = plt.colorbar(sm, ax=ax)
cbar.set_label("Hidden Dimension")
# Set colorbar ticks to show width values
cbar.set_ticks([w for w in width])
cbar.set_ticklabels([str(w) for w in width])
...
pattern = re.compile(rf"h_(\d+)_job\d+test_set_e{epoch}.pt$")
