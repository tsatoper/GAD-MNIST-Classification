...
model_dir = 'recreate_mse'
xscale = 'log'
epoch = 100
sv_directory = f"/glade/derecho/scratch/tsatoperry/GAD/MNIST/models/{model_dir}/singular_values"
metrics_dir = f'/glade/derecho/scratch/tsatoperry/GAD/MNIST/models/{model_dir}/metrics/'
...
if os.path.exists(metrics_dir):
    for fname in os.listdir(metrics_dir):
        match = re.match(r'w(\d+)_job(\d+)\.json', fname)
        if match:
...
# ===== Load Test SV Data =====
pattern = re.compile(rf"w(\d+)_job(\d+)test_e{epoch}.pt$")
...
ax1.set_title(f"Singular Values MNIST ({model_dir})")
ax1.set_xlabel("Index")
ax1.set_ylabel("Singular Value")
ax1.set_xscale(xscale)
ax1.set_yscale("log")
# ax1.set_ylim(bottom=1e-8, top=None)
...
ax2.set_title(f"Loss Values MNIST ({model_dir}). {samples} samples, epoch {epoch}")
