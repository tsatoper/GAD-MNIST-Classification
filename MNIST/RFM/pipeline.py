...
width = 50 #[5*i for i in range(1, 21)][args.array_idx]
...
for epoch in range(1, num_epochs + 1):
    ...
    if epoch in save_at_this_epoch:
        ...
        # Save model weights
        weights_path = f'{args.output_dir}/weights/{filename}_e{epoch}.pth'
        torch.save(model.state_dict(), weights_path)
        print(f"Model weights saved at epoch {epoch} to{weights_path}")
        json_input[f'epoch{epoch}_weight_path'] = weights_path
