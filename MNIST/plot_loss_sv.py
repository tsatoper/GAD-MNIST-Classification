...
# ====== CONFIGURATION ======
plot_id = 'N1_1e-4'
epochs = [100, 2000]# list(range(50, 2000, 50))
yscale = 'log'
...
def load_sv_mean_from_path(sv_path):
    """Load a .pt singular values file and return the mean, or None on failure."""
    try:
        sv_tensor = torch.load(sv_path, map_location='cpu')
        # Handle various tensor shapes: scalar, 1-D, or 2-D (per-layer stacked)
        sv_tensor = sv_tensor.float()
        return sv_tensor.mean().item()
    except Exception as e:
        print(f"    Could not load SV from {sv_path}: {e}")
        return None


def get_sv_mean(data, sv_key, acts_path_key):
    """
    Try sv_key first; if missing/None, fall back to loading from acts_path_key.
    acts_path_key points to a list [activations_path, sv_path]; we use index [1].
    Returns float or None.
    """
    # Primary: pre-computed scalar in JSON
    value = data.get(sv_key, None)
    if value is not None:
        return value

    # Fallback: load from .pt file
    paths = data.get(acts_path_key, None)
    if paths is not None and isinstance(paths, (list, tuple)) and len(paths) >= 2:
        sv_path = paths[1]
        print(f"    Falling back to .pt file for key '{sv_key}': {sv_path}")
        return load_sv_mean_from_path(sv_path)
...
for filename in os.listdir(dir_name):
    ...
    try:
        ...
        for epoch in epochs:
            ...
            acts_path_key   = f'epoch{epoch}_acts_path'   # fallback key

            # Require at least loss keys
            if train_key not in data or test_key not in data:
                print(f"  [{filename}] Missing loss keys for epoch {epoch}, skipping.")
                continue

            sv_train = get_sv_mean(data, sv_train_key, acts_path_key)
            sv_test  = get_sv_mean(data, sv_test_key,  acts_path_key)

            # Skip epoch if neither SV is available
            if sv_train is None and sv_test is None:
                print(f"  [{filename}] No SV data for epoch {epoch}, skipping.")
                continue
