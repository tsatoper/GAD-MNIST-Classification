...
for filename in os.listdir(dir_name):
    ...
    try:
        ...
        for epoch in epochs:
            ...
            sv_data = torch.load(sv_filepath, weights_only=True)
            sv_train_mean = float(sv_data['train'].mean())
            sv_test_mean  = float(sv_data['test'].mean())
            ...
            all_data[epoch]['sv_train'].append(sv_train_mean)
            all_data[epoch]['sv_test'].append(sv_test_mean)
...
all_sv_combined = np.concatenate([all_sv_train, all_sv_test])

loss_ymin = min(np.min(all_test), np.min(all_train)) * 0.8
loss_ymax = max(np.max(all_test), np.max(all_train)) * 1.2
sv_ymin   = np.min(all_sv_combined) * 0.8
sv_ymax   = np.max(all_sv_combined) * 1.2
...
for epoch in epochs:
    ...
    # Right axis: train and test mean SV
    ax2 = ax1.twinx()
    for i, w in enumerate(width):
        color = cmap(norm(w))
        if i < len(width) - 1:
            ax2.plot(width[i:i+2], sv_train[i:i+2], linestyle='-.',          linewidth=2.5, alpha=0.7, color=color)
            ax2.plot(width[i:i+2], sv_test[i:i+2],  linestyle=(0,(3,1,1,1)), linewidth=2.5, alpha=0.7, color=color)
        ax2.scatter(w, sv_train[i], s=60, alpha=0.7, marker='s', zorder=5, color=color)
        ax2.scatter(w, sv_test[i],  s=60, alpha=0.7, marker='D', zorder=5, color=color)
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color='gray', linewidth=2.5, linestyle='-.',          label='Train Mean SV'),
        Line2D([0], [0], color='gray', linewidth=2.5, linestyle=(0,(3,1,1,1)), label='Test Mean SV'),
